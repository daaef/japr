...
export default defineEventHandler(async (event) => {
  ...
  const reviews = await db
    ...
    .where(and(
      eq(reviewers.userId, session.user.id),
      or(
        eq(journals.approvalStatus, MANUSCRIPT_STATUS.APPROVED),
        eq(journals.approvalStatus, MANUSCRIPT_STATUS.APPROVED_WITH_COMMENT)
      )
    ))
